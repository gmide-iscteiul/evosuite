...
public class WhaleOptimizationAlgorithm<T extends Chromosome<T>> extends GeneticAlgorithm<T> {
	...
	protected void evolve() {
		...
		for (int i = 1; i < population.size(); i++) {
			...
			/*
			 * A- exploration -> crossover with random whale || exploitation -> crossover with best whale
			 * C- mutation
			 * Spiral movement -> mutation + crossover with best whale? 
			 */
			...
			try {
				if (p < 0.5) {
					...
					if (C >= 2 * Properties.MUTATION_RATE) { // both 2.1 & 2.8 depend on C
						notifyMutation(whale);
						whale.mutate();
					}
				} else { // p >= 0.5
					// eq 2.5 spiral
					//double l = Randomness.nextDouble(-1.0, 1.0);
					notifyMutation(whale);
					whale.mutate();
					crossoverFunction.crossOver(whale, bestWhale.clone());
				}
